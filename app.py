...
def call_llm_text(provider: str, model: str, prompt: str, temperature: float = 0.2, max_tokens: int = 3000) -> str:
    try:
        if provider == "Gemini" and google_genai:
            ...
            return "LLM not configured. Please provide API key."
    except Exception as e:
...
with tab_home:
    ...
    with cols[1]: status_chip(f"OCR: {'✅' if pdfplumber or (pdf2image and pytesseract) else '⚠️'}", "#FFB020")
    with cols[2]: status_chip(f"Provider: {provider}", "#3388FF")
    with cols[3]: status_chip(f"Theme: {st.session_state.graph_theme}", "#AA66FF")
...
with tab_prompt:
    ...
    with col2:
        ...
        if use_openai_key and not openai_key:
            st.warning("⚠️ Please enter OpenAI API key in sidebar")
    ...
    execute_col1, execute_col2, execute_col3 = st.columns([1, 1, 2])
...
with tab_agents:
    ...
    if not cfg:
        ...
        if not names:
            ...
            agent_provider = st.selectbox("Provider", ["Gemini", "OpenAI", "Grok"], index=0)
            if agent_provider == "Gemini":
                agent_model = st.selectbox("Model", ["gemini-2.0-flash-exp", "gemini-1.5-flash"])
            elif agent_provider == "OpenAI":
                agent_model = st.selectbox("Model", ["gpt-4o", "gpt-4o-mini"])
            else:
                agent_model = st.selectbox("Model", ["grok-beta", "grok-vision-beta"])
